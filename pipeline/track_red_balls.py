...
class TrackRedBalls(Pipeline):

    def map(self, data):

        ROI = data["ROI"]
        LAB_ROI = data["LAB_ROI"]
        data["red_balls_count"] = 0

        font = cv2.FONT_HERSHEY_SIMPLEX

        lower = np.array([30, 180, 0], dtype=np.uint8)
        upper = np.array([255, 213, 160], dtype=np.uint8)
        ...
        for ctns in contours:
            ...
            if rw > 20 and rw < 40 and rh > 15 and rh < 35:
                data["red_balls_count"] = data["red_balls_count"] + 1
